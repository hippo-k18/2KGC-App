 ...
 *
 * Two separate things are missing here and they have very different sizes.
 *
 * The first is the one Message Exhibitors used to hit and no longer does:
 * `AudienceId` in `src/lib/messaging.ts` now takes a third value and resolves
 * exhibitors, so a fourth would be the same small edit.
 *
 * The second is the real blocker: **there is nobody to send to.**
 ...
 * and no invitation flow anywhere in this dashboard, and building one means
 * reconciling the free-text names already on tasks against it — the awkward
 * part, and it does not get easier with time.
 *
 * Worth naming the cheaper alternative: for six people running a conference, a
 * group in somebody's mail client works and costs nothing. This screen earns its
 * keep at twenty-plus volunteers, not at six.
 */
    ...
    <>
        ...
        {assignees.length === 0 ? (
          ...
          <Table
            ...
            rows={assignees.map((name) => [
              ...
              <span key="o">
                {tasks.filter((t) => t.assignee === name && t.status !== 'done').length}
              </span>,
      ...
      <GapPanel style={{ marginTop: 16 }}>
        ...
        <ul className="muted" style={{ fontSize: 13, lineHeight: 1.7, marginBottom: 0 }}>
          <li>
            <strong>A team model.</strong> A collection of organizers and helpers with a name, an
            address and a role, an invitation flow, and a migration reconciling the free-text names
            already on tasks against it.
          </li>
          <li>
            <strong>Sending.</strong> No compose box, no send, no history — downstream of the
            model above, and one small edit to <code>AudienceId</code> once it exists.
          </li>
          <li>
          ...
          <li>
            <strong>Per-role permissions.</strong> One role: organizer or not.
          </li>
          <li>
