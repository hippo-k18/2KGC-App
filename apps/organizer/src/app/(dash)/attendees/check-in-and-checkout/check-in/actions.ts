...
/**
 * Check somebody in from the attendee table, with no code at all.
 *
 * This is the gap that mattered. The scanner needs a `qrSecret` or a
 * `claimCode`, and both live on the attendee's phone — so a flat battery, a
 * cracked screen or someone who never installed the app was, until now,
 * un-checkinable. Whova's running check-in screen is a searchable table with an
 * inline "Check in" button per row for exactly this reason, and a queue of a
 * thousand people will produce that case within the first hour.
 *
 * Same write as the scanner: `create()` on a registration-keyed document, so a
 * double click is an `already-exists` and not a double count. No `scanEvents`
 * row is written, because nothing was scanned — the audit log is where a manual
 * desk action belongs, and it records who did it.
 */
export async function checkInByIdAction(
  listId: string,
  registrationId: string,
): Promise<DeskState> {
  const actor = await requireOrganizer();
  if (!listId || !registrationId) return { error: 'Missing list or registration.' };
  ...
  try {
    ...
    if (created) {
      await appendAudit({
        ...
        targetPath: `${COLLECTIONS.checkInLists}/${listId}/${SUBCOLLECTIONS.checkIns}/${registrationId}`,
        targetId: registrationId,
        before: {},
        after: { stationId: 'dashboard' },
      });
    }
    ...
    revalidatePath(ROUTES.warRoom);
    return created
      ? { ok: true, message: 'Checked in.' }
      : { ok: true, message: 'Already checked in.' };
  } catch (err) {
    recordError('checkin.manual', err);
    return { error: err instanceof Error ? err.message : 'Could not check in.' };
  }
}
 ...
 * Undo a check-in.
 *
 * Whova has this and we did not, which made every mistake permanent: a
 * mis-tapped row, a scan of the wrong badge, or a test run against the live
 * list left a person marked present with no way back. The deletion is audited
 * rather than silent, because "who was here" is the number the fire marshal
 * asks about and an unexplained decrement is worse than none.
 *
 * Note this deliberately does not touch `scanEvents`. That log is append-only
 * and records what was *scanned*, which remains true even after the check-in it
...
export async function undoCheckInAction(
  ...
  try {
    ...
    const snap = await ref.get();
    if (!snap.exists) return { ok: true, message: 'Not checked in.' };
    ...
    recordError('checkin.undo', err);
    return { error: err instanceof Error ? err.message : 'Could not undo.' };
  }
}
