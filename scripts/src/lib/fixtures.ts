...
export interface SeedSpeaker {
  ...
  /** Lifted into the "Our First Speakers" block at the top of `/speakers`. */
  featured: boolean;
  /** Position on the published roster, ascending. */
  displayOrder: number;
}

/**
 * How many of the seeded speakers are highlighted, and which.
 *
 * Whova's live widget highlighted exactly five, named outright in its payload
 * as `design.highlight_speakers`, and the public page is built around that
 * shape: three cards, then two centred beneath them, then everyone else behind
 * "View All Speakers". Reproducing the page therefore means reproducing the
 * five, not just the grid.
 *
 * They are chosen by an even stride rather than by taking the first five.
 * Taking the first five would highlight whoever the surname sort happened to
 * put at the top — the objection that kept `featured` off `SpeakerDoc` for as
 * long as it was off — and a demo whose five highlights are four Adamses and a
 * Bergström reads as a bug rather than as an editorial choice. A stride spreads
 * them across the alphabet the way a real programme committee's picks are.
 */
...
export function makeSpeakers(n: number): SeedSpeaker[] {
  ...
  /*
   * `displayOrder` is assigned over the surname sort, not over generation
   * order, because the public page is a roster and a roster is alphabetical.
   * The two orders differ: names are generated by striding through FIRST and
   * LAST, so generation order is essentially arbitrary. Sorting first and
   * numbering second is what makes position 0 mean "first on the page".
   */
  const stride = Math.max(1, Math.floor(n / FEATURED_COUNT));
  ...
  const surname = (nm: string) => nm.split(/\s+/).pop()!.toLowerCase();
  out.sort((a, b) => surname(a.name).localeCompare(surname(b.name)) || a.name.localeCompare(b.name));
  out.forEach((s, i) => {
    s.displayOrder = i;
    s.featured = i % stride === 0 && i / stride < FEATURED_COUNT;
  });
